...
def get_original_id_map():
    id_file = open('original_ids.txt', 'r')
    lines = id_file.readlines()
    id_file.close()
    id_map = {}
    for i in range(len(lines)):
        id_map[int(lines[i])] = i
    return id_map

def get_machine_guesses(numLangs, labelDict):
    predFile = open('predictions/predictions_' + str(numLangs) + '.txt')
...
def accent_guesser_loop(wav_dir, labelDict):
    ...
    original_id_map = get_original_id_map()
    ...
    while ask_user_to_continue(stats):
        ...
        machineGuess = machineDict[original_id_map[speaker_id]]
