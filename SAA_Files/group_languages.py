...
def group_langs(bio_filename, movs_dir, out_dir, num_langs, samples_per_lang):
	...
	for i in range(num_langs):
		...
		for j in range(len(speaker_ids)):
			...
			if genders[j] == 'male':
				numMale += 1
				increment = copy_sample(movs_dir, speaker_ids[j], lang_path, "male"+str(numMale)+".mov")
			elif genders[j] == 'female':
				numFemale += 1
				increment = copy_sample(movs_dir, speaker_ids[j], lang_path, 'female'+str(numFemale)+".mov")
			numRead += increment
